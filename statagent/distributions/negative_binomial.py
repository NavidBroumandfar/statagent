...
class NegativeBinomialAnalyzer:
    ...
    def find_significant_range(
        ...
        x_max : int, optional
            Maximum value to inspect (default: 400)
        ...
        if x_max < 0:
            raise ValueError("x_max must be non-negative")

        x_vals, pmf_vals = self.compute_pmf(x_max=x_max)
        mask = pmf_vals >= threshold
        indices = x_vals[mask]

        if len(indices) == 0:
            raise ValueError(
                "No PMF values meet the threshold within the inspected range"
            )
