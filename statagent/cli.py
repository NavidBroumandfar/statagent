...
def survival_mixture_cli(args):
    ...
    model = SurvivalMixtureModel(
        w1=args.w1, rate1=args.rate1, w2=args.w2, rate2=args.rate2
    )
...
def hypothesis_test_cli(args):
    ...
    print(
        f"  Decision: {'Reject H0' if result['reject_null'] else 'Fail to reject H0'}"
    )
...
def main():
    ...
    nb_parser.add_argument(
        "--range", action="store_true", help="Show significant range"
    )
    nb_parser.add_argument(
        "--threshold", type=float, default=0.005, help="Threshold for range"
    )
    ...
    sm_parser.add_argument(
        "--probability", action="store_true", help="Compute probability"
    )
    ...
    sm_parser.add_argument(
        "--n-samples", type=int, default=100000, help="Number of samples"
    )
    ...
    ht_parser.add_argument(
        "--mu-0", type=float, required=True, help="Null hypothesis mean"
    )
    ht_parser.add_argument(
        "--sigma", type=float, required=True, help="Population std dev"
    )
    ht_parser.add_argument(
        "--test-type",
        choices=["left", "right", "two"],
        default="two",
        help="Type of test",
    )
    ht_parser.add_argument(
