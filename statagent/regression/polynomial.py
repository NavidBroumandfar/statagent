...
class PolynomialRegression:
    ...
    def __init__(self, degree: int = 10):
        ...
        self.ols_rank_deficient_ = False
        self._fitted = False
    ...
    def fit(
        self, x: np.ndarray, y: np.ndarray, lambda_ridge: float = 1e4
        ...
        if lambda_ridge < 0:
            raise ValueError("lambda_ridge must be non-negative")

        # Create design matrix
        X = self._create_vandermonde(x)

        # OLS solution. Use least squares rather than inverting X'X directly;
        # high-degree polynomials are often rank deficient.
        XtX = X.T @ X
        Xty = X.T @ y
        ...
        try:
            ...
            self.coef_ols_ = coef
            self.ols_rank_deficient_ = bool(rank < X.shape[1])
        except np.linalg.LinAlgError:
            self.coef_ols_ = np.full(self.degree + 1, np.nan)
            self.ols_rank_deficient_ = True

        # Ridge solution: alpha = (X'X + λI)^{-1} X'y
        try:
            self.coef_ridge_ = np.linalg.solve(
                XtX + lambda_ridge * np.eye(self.degree + 1), Xty
            )
        except np.linalg.LinAlgError:
...
Model Configuration:
  ...
  Rank deficient: {self.ols_rank_deficient_}
  MSE: {mse_ols:.2e}
