"""
Statistical Agent - Main interface for agent-assisted statistical analysis.

This module provides the main StatisticalAgent class that orchestrates
data profiling, method selection, workflow execution, and interpretation.
"""
...
class StatisticalAgent:
    """
    Agent-assisted statistical analysis interface.

    The StatisticalAgent examines data, reasons about appropriate methods,
    executes selected analyses, and interprets results.
    ...
    def analyze(
        ...
        **kwargs,
        ...
        """
        Perform agent-assisted statistical analysis.
        ...
        **kwargs
        ...
        orchestrator = Orchestrator(data=examiner.data, verbose=self.verbose)
        ...
        recommendations = interpreter.suggest_next_steps(results, goal, data_profile)
    ...
    def _rule_based_method_selection(
        self, data_profile: Dict, goal: str, hypothesis: Optional[str] = None
    ) -> Dict:
        """
        ...
        if (
            "estimate" in goal.lower()
            or "parameter" in goal.lower()
            or "comprehensive" in goal.lower()
        ):
