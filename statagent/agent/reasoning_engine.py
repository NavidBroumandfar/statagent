...
class ReasoningEngine:
    ...
    def _setup_ollama(self):
        ...
        try:
            ...
            self.ollama_model = (
                self.llm.split("/", 1)[1] if "/" in self.llm else "llama3"
            )
        except ImportError:
            raise ImportError(
                "Ollama package not installed. Install with: pip install -e '.[llm]'"
            )
    ...
    def select_methods(
        self, data_profile: Dict, goal: str, data_analysis: Optional[str] = None
    ) -> Dict:
        """
    ...
    def interpret_results(
        self, method_name: str, parameters: Dict, results: Any, goal: str
    ) -> str:
        """
    ...
    def plan_workflow(self, data_profile: Dict, goal: str) -> Dict:
        ...
        prompt = WORKFLOW_PLANNING_PROMPT.format(
            data_profile=json.dumps(data_profile, indent=2), goal=goal
        )
    ...
    def recover_from_error(
        self, method_name: str, parameters: Dict, error: str, data_profile: Dict
    ) -> Dict:
        """
