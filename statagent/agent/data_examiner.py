...
class DataExaminer:
    ...
    def _prepare_data(
        self, data: Union[np.ndarray, pd.Series, pd.DataFrame, list]
    ) -> np.ndarray:
        """Convert data to numpy array and handle basic cleaning."""
        try:
            if isinstance(data, pd.DataFrame):
                if data.shape[1] == 1:
                    data = data.iloc[:, 0].to_numpy(dtype=float)
                else:
            ...
            raise ValueError("DataExaminer requires numeric data") from exc
    ...
    def _compute_basic_stats(self) -> Dict:
        ...
        stats_dict = {
            ...
            "variance": (
                float(np.var(clean_data, ddof=1)) if len(clean_data) > 1 else 0.0
            ),
    ...
    def _check_data_quality(self) -> Dict:
        ...
        quality = {
            ...
            "missing_percentage": (
                len(self.data) - len(self.data[~np.isnan(self.data)])
            )
            / len(self.data)
            * 100,
        ...
        quality["is_constant"] = (
            len(np.unique(clean_data)) == 1 if len(clean_data) > 0 else True
        )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if data_type == "discrete_count":
            if distribution.get("overdispersed", False):
                recommendations.append(
                    "Consider Negative Binomial for overdispersed count data"
                )
            else:
                recommendations.append(
                    "Poisson or Negative Binomial may be appropriate"
                )

        if data_type == "continuous_positive":
            recommendations.append(
                "Consider exponential, gamma, or log-normal distributions"
            )
        ...
        if data_type == "continuous":
            if distribution.get("appears_normal", False):
                recommendations.append(
                    "Data appears normally distributed - parametric tests suitable"
                )
            else:
                recommendations.append(
        ...
        if outliers.get("percentage", 0) > 5:
            recommendations.append(
                "Significant outliers detected - consider robust methods"
            )
        ...
        if distribution.get("dispersion_ratio"):
            ...
            if ratio > 2:
                recommendations.append(
                    "High dispersion - consider models that account for overdispersion"
                )
