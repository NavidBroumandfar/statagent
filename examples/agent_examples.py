...
This file demonstrates the experimental StatisticalAgent workflow:
...
The default examples use rule-based method selection so they can run without an
API key.
"""
...
def example_count_data():
    ...
    # Agent analyzes with rule-based method selection
...
def example_hypothesis_testing():
    ...
    # Agent selects and executes a hypothesis test
    report = agent.analyze(data=data, goal="test_hypothesis", hypothesis="mean > 1000")
...
def example_continuous_data():
    ...
    report = agent.analyze(data=data, goal="comprehensive_statistical_analysis")
...
def example_with_llm():
    ...
    if not api_key:
        ...
        print("\n[OK] OpenAI API key found. Using optional LLM reasoning.")
        use_llm = True

    # Mixed data (challenging to classify)
    np.random.seed(42)
    data = np.concatenate(
        [np.random.poisson(lam=5, size=30), np.random.poisson(lam=15, size=20)]
    )

    print(f"\nData: {len(data)} observations (mixture of two processes)")

    # Create agent with LLM
    agent = StatisticalAgent(llm="gpt-4", verbose=True, use_llm=use_llm)

    # Agent reasons about the data
    report = agent.analyze(
        data=data, goal="identify_distribution_and_estimate_parameters"
    )
...
def main():
    ...
    print("=" * 80)
    print("\nThese examples demonstrate the agent-assisted analysis workflow:")
